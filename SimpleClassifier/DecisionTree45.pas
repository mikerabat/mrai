...
begin
     ...
     begin
          // randomize the given set two times
          for i := 0 to 2*length(weights) - 1 do
          begin
               idx1 := random(Length(weights));
               idx2 := random(length(weights));

               hlp := trainingIdx[idx1];
               trainingIdx[idx1] := trainingIdx[idx2];
               trainingIdx[idx2] := hlp;
          end;
